 ...
 * Se carga una sola vez con cargarDatosIniciales(); si la hoja ya tiene filas,
 * la funcion no toca nada (es segura de volver a ejecutar).
 *
 * Convenciones aplicadas sobre la fuente original, todas reversibles desde la
 * pagina de Administracion:
 *   - El ID de iniciativa se prefija con "INI-" porque Google Sheets convierte
 *     el texto "001" en el numero 1 y se perderia el formato de la llave.
 *   - La plataforma "Portal Empresarial" llegaba con el mismo ID que "Shivam"
 *     (PL-07); se le asigno PL-08.
 *   - El estado de la iniciativa usa el catalogo de Estados (EST-01 Por iniciar,
 *     EST-02 En progreso), en vez de un texto libre.
 *   - Los Business Owner entregados por nombre quedan creados como Usuarios con
 *     un correo marcado "@pendiente" hasta que se confirme el dominio.
 */

/** Catalogo real de plataformas digitales. */
...
/** Business Owners entregados con la data. Correo pendiente de confirmar. */
var USUARIOS_SEMILLA = [
  { correo: 'claudia.gomez@pendiente', nombre: 'Claudia Gomez', cargo: 'Business Owner', area: 'Negocio', rol: 'RO-01' },
  { correo: 'mauricio.osorio@pendiente', nombre: 'Mauricio Osorio', cargo: 'Business Owner', area: 'Negocio', rol: 'RO-01' },
];

/**
 * Las 39 iniciativas del portafolio, en el orden entregado.
 * Los campos vacios se cargan vacios: no se inventan valores.
 */
var INICIATIVAS_REALES = [
  { id: 'INI-001', nombre: 'Evolución y Mantenimiento Analizamos', descripcion: '', prioridad: 'Media', tipo: 'TIN-01',
    len: 'LEN-00', vertical: 'VER-00', bo: '', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-02' },
  ...
  { id: 'INI-013', nombre: 'Hallazgos de auditoría Analizamos', descripcion: '', prioridad: 'Media', tipo: 'TIN-02',
    len: 'LEN-00', vertical: 'VER-01', bo: '', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-01' },
  ...
  { id: 'INI-016', nombre: 'Iniciativa UIN en Banca Móvil', descripcion: '', prioridad: 'Alta', tipo: 'TIN-04',
    len: 'LEN-00', vertical: 'VER-02', bo: '', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-01' },
  ...
  { id: 'INI-026', nombre: 'Proyecto Data Lake House', descripcion: '', prioridad: 'Crítica', tipo: 'TIN-03',
    len: 'LEN-00', vertical: 'VER-00', bo: 'claudia.gomez@pendiente', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-01' },
  ...
  { id: 'INI-028', nombre: 'Santander Agro 360', descripcion: '', prioridad: 'Alta', tipo: 'TIN-01',
    len: 'LEN-02', vertical: 'VER-00', bo: '', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-01' },
  { id: 'INI-029', nombre: 'Apertura de CDT con MejorCDT', descripcion: '', prioridad: 'Alta', tipo: 'TIN-01',
    len: 'LEN-00', vertical: 'VER-02', bo: 'mauricio.osorio@pendiente', po: '',
    fechaEstimada: '', fechaInicio: '14/09/2026', fechaFin: '', estado: 'EST-02' },
  { id: 'INI-030', nombre: 'Hub Digital Crezcamos', descripcion: 'Rediseño página Web', prioridad: 'Crítica', tipo: 'TIN-04',
    len: 'LEN-00', vertical: 'VER-00', bo: 'mauricio.osorio@pendiente', po: '',
    fechaEstimada: '', fechaInicio: '', fechaFin: '', estado: 'EST-02' },
...
/**
 * Carga catalogos y iniciativas en el libro de parametrizacion.
 * Solo escribe en las hojas que estan vacias.
...
function cargarDatosIniciales() {
  ...
  cargarSiVacio_(libro, 'Usuarios', USUARIOS_SEMILLA.map(function (u) {
    return [u.correo, u.nombre, u.cargo, u.area, u.rol, 'SI'];
  }), resumen);
  ...
  cargarSiVacio_(libro, 'Proyectos', INICIATIVAS_REALES.map(function (i) {
    return [i.id, i.nombre, i.descripcion, i.prioridad, i.tipo, i.bo, i.po,
            i.len, i.vertical, i.fechaEstimada, i.fechaInicio, i.fechaFin,
            i.estado, ''];
  }), resumen);
